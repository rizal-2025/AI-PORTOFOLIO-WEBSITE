...
export default async function AboutPage() {
  ...
  return (
    <>
      <section data-section-id={aboutSectionIds[0]} className="relative isolate overflow-hidden border-b border-zinc-800 bg-[#07070b]">
        <div className="matrix-grid absolute inset-0 -z-20" aria-hidden="true" /><div className="absolute -left-40 top-0 -z-10 size-[30rem] rounded-full bg-violet-500/10 blur-[110px]" aria-hidden="true" />
        ...
        <PageContainer className="grid gap-12 pb-20 sm:pb-28 lg:grid-cols-[minmax(0,1fr)_23rem] lg:items-end"><div><p className="inline-flex items-center gap-2 rounded-full border border-violet-400/20 bg-violet-400/5 px-3 py-1.5 font-mono text-xs font-semibold uppercase tracking-[0.16em] text-violet-300"><span className="size-1.5 rounded-full bg-violet-400" aria-hidden="true" />{copy.heroEyebrow}</p><h1 className="mt-6 max-w-4xl text-4xl font-semibold leading-[1.06] tracking-[-0.045em] text-zinc-50 sm:text-6xl">{copy.heroTitle} <span className="text-violet-300">{copy.heroAccent}</span></h1><p className="mt-6 max-w-2xl text-base leading-8 text-zinc-400 sm:text-lg">{copy.heroSummary}</p><div className="mt-7 flex flex-wrap gap-2"><span className="rounded-full border border-violet-400/20 bg-violet-400/5 px-3 py-1.5 font-mono text-xs text-violet-300">{copy.status}</span><span className="rounded-full border border-zinc-700 bg-zinc-900/70 px-3 py-1.5 font-mono text-xs text-zinc-400">{siteConfig.owner.availability ?? copy.profileFallbacks[3]}</span></div><div className="mt-9 flex flex-col gap-3 sm:flex-row"><ButtonLink href="/projects">{copy.viewProjects}</ButtonLink><ButtonLink href="/contact" variant="secondary">{copy.contactMe}</ButtonLink></div></div><TerminalPanel label={copy.profileLabel}><dl className="grid gap-3 p-5 font-mono text-xs sm:text-sm">{copy.profileRows.map(([label, value]) => <div key={label} className="flex items-start justify-between gap-5 border-b border-zinc-800 pb-3 last:border-0 last:pb-0"><dt className="text-zinc-500">{label}</dt><dd className="text-right text-violet-300">{value}</dd></div>)}</dl></TerminalPanel></PageContainer>
      </section>

      <section data-section-id={aboutSectionIds[1]} className="bg-[#0b0a0f] py-16 sm:py-24"><PageContainer><div className="grid gap-12 lg:grid-cols-[0.7fr_1.3fr] lg:gap-20"><div><p className="font-mono text-xs font-semibold uppercase tracking-[0.2em] text-violet-300">{copy.summaryEyebrow}</p><h2 className="mt-4 text-3xl font-semibold tracking-[-0.03em] text-zinc-50 sm:text-4xl">{copy.summaryTitle}</h2></div><div className="max-w-2xl space-y-5 text-base leading-8 text-zinc-400">{copy.summaryParagraphs.map((paragraph) => <p key={paragraph}>{paragraph}</p>)}</div></div><div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{copy.strengths.map(([title, description, category], index) => <article key={title} className="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-6"><div className="flex items-center justify-between"><span className="grid size-10 place-items-center rounded-xl border border-violet-400/15 bg-violet-400/5 text-violet-300"><svg viewBox="0 0 24 24" className="size-5" fill="none" stroke="currentColor" strokeWidth="1.6" aria-hidden="true"><path d="M5 12h14M12 5v14" /><circle cx="12" cy="12" r="8" /></svg></span><span className="font-mono text-xs text-zinc-600">{String(index + 1).padStart(2, "0")}</span></div><h3 className="mt-6 text-lg font-semibold text-zinc-100">{title}</h3><p className="mt-3 text-sm leading-6 text-zinc-400">{description}</p><p className="mt-5 font-mono text-[0.65rem] uppercase tracking-[0.1em] text-violet-300">{category}</p></article>)}</div></PageContainer></section>

      <section data-section-id={aboutSectionIds[2]} className="border-y border-zinc-800 bg-[#07070b] py-16 sm:py-24"><PageContainer className="grid gap-12 lg:grid-cols-2 lg:gap-16"><div><p className="font-mono text-xs font-semibold uppercase tracking-[0.2em] text-violet-300">{copy.approachEyebrow}</p><h2 className="mt-4 text-3xl font-semibold tracking-[-0.03em] text-zinc-50">{copy.approachTitle}</h2><ul className="mt-7 grid gap-4 text-sm leading-6 text-zinc-400">{copy.approach.map((item, index) => <li key={item} className="flex gap-3"><span className="grid size-7 shrink-0 place-items-center rounded-lg border border-violet-400/15 bg-violet-400/5 font-mono text-xs text-violet-300">{String(index + 1).padStart(2, "0")}</span>{item}</li>)}</ul></div><TerminalPanel label={copy.currentProfile}><dl className="grid gap-3 p-5 text-sm">{copy.profileLabels.map((label, index) => <div key={label} className="flex justify-between gap-5"><dt className="text-zinc-500">{label}</dt><dd className={index === 1 ? "text-violet-300" : "text-zinc-200"}>{profileValues[index]}</dd></div>)}</dl></TerminalPanel></PageContainer></section>

      <section data-section-id={aboutSectionIds[3]} className="bg-[#0b0a0f] py-16 sm:py-24"><PageContainer><p className="font-mono text-xs font-semibold uppercase tracking-[0.2em] text-violet-300">{copy.projectEyebrow}</p><div className="mt-6 grid gap-8 rounded-3xl border border-violet-400/15 bg-zinc-900/50 p-7 sm:p-10 lg:grid-cols-[0.8fr_1.2fr]"><div><p className="font-mono text-sm text-violet-300">{copy.primarySystem}</p><h2 className="mt-3 text-4xl font-semibold tracking-[-0.04em] text-zinc-50">AURA</h2><p className="mt-4 text-sm leading-7 text-zinc-400">{copy.projectSummary}</p></div><div><p className="text-base leading-8 text-zinc-300">{copy.projectDescription}</p><div className="mt-7 flex flex-col gap-3 sm:flex-row"><ButtonLink href="/projects/aura">{copy.viewCaseStudy}</ButtonLink><ButtonLink href="/architecture" variant="secondary">{copy.viewArchitecture}</ButtonLink><ButtonLink href="/demo/aura" variant="secondary">{copy.tryDemo}</ButtonLink></div></div></div></PageContainer></section>

      <section data-section-id={aboutSectionIds[4]} className="border-y border-zinc-800 bg-[#07070b] py-16 sm:py-24"><PageContainer className="grid gap-12 lg:grid-cols-2"><div><p className="font-mono text-xs font-semibold uppercase tracking-[0.2em] text-violet-300">{copy.technologyEyebrow}</p><div className="mt-7 grid gap-3">{copy.technologies.map(([title, stack, description]) => <article key={title} className="rounded-xl border border-zinc-800 bg-zinc-900/50 p-5"><p className="font-mono text-xs text-violet-300">{title}</p><h2 className="mt-3 text-lg font-semibold text-zinc-100">{stack}</h2><p className="mt-2 text-sm leading-6 text-zinc-400">{description}</p></article>)}</div></div><div><p className="font-mono text-xs font-semibold uppercase tracking-[0.2em] text-violet-300">{copy.principlesEyebrow}</p><div className="mt-7 grid gap-3">{copy.principles.map((item) => <article key={item} className="rounded-xl border border-zinc-800 bg-zinc-900/50 px-5 py-4 text-sm font-medium text-zinc-300"><span className="mr-3 font-mono text-violet-300">{"//"}</span>{item}</article>)}</div></div></PageContainer></section>

      <section data-section-id={aboutSectionIds[5]} className="bg-[#0b0a0f] py-16 sm:py-24"><PageContainer><p className="font-mono text-xs font-semibold uppercase tracking-[0.2em] text-violet-300">{copy.directionEyebrow}</p><div className="mt-8 grid gap-5 lg:grid-cols-2"><article className="rounded-2xl border border-violet-400/15 bg-violet-400/[0.035] p-7"><h2 className="text-xl font-semibold text-zinc-100">{copy.buildingTitle}</h2><ul className="mt-5 grid gap-3 text-sm text-zinc-400">{copy.building.map((item) => <li key={item} className="flex gap-3"><span className="mt-2 size-1.5 shrink-0 rounded-full bg-violet-400" aria-hidden="true" />{item}</li>)}</ul></article><article className="rounded-2xl border border-zinc-800 bg-zinc-900/50 p-7"><h2 className="text-xl font-semibold text-zinc-100">{copy.goalsTitle}</h2><ul className="mt-5 grid gap-3 text-sm text-zinc-400">{goals.map((item) => <li key={item} className="flex gap-3"><span className="mt-2 size-1.5 shrink-0 rounded-full bg-violet-400" aria-hidden="true" />{item}</li>)}</ul></article></div><div className="mt-10 rounded-3xl border border-violet-400/20 bg-zinc-900 px-7 py-10 sm:px-10"><h2 className="text-3xl font-semibold tracking-[-0.03em] text-zinc-50">{copy.finalTitle}</h2><div className="mt-7 flex flex-col gap-3 sm:flex-row"><ButtonLink href="/contact">{copy.contactMe}</ButtonLink><ButtonLink href="/projects" variant="secondary">{copy.viewProjects}</ButtonLink></div></div></PageContainer></section>
    </>
  );
}
